 ...
 * @param {object} customAssets - window.customAssets object
 * @returns {number} - The scale to apply to the hero image
 */
function calculateHeroScale(assetKey, config, customAssets) {
  const scaleFactor = config.gameplay?.scaleFactor || 1.4;
  const userScale = config.assetScales?.[assetKey] || 1;

  // Detect if using custom asset (uploaded by user) vs default template asset
  // Custom assets come from: customAssets object, or URLs starting with http/https/data:
  const customAssetUrl = customAssets?.[assetKey];
  const configAssetUrl = config.assets?.[assetKey] || '';

  const isCustomAsset = customAssetUrl ||
    configAssetUrl.startsWith('http') ||
    configAssetUrl.startsWith('data:');

  if (isCustomAsset) {
    // Custom assets: WYSIWYG - direct scale only, no magic formulas
    // User sets scale=1.0 means original image size
    return userScale;
  } else {
    // Default template assets: use original formula designed for template showcase
    // This formula was tuned for the default carTitle.png dimensions (640x436)
    const heroScale = (config.gameplay?.carTitleScale || 1.54) * scaleFactor;
    return heroScale * userScale;
  }
}
...
class SplashScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Hero car (bottom) - Uses calculateHeroScale for WYSIWYG custom assets
    if (config.layout?.showSplashHero && this.textures.exists('splashHero')) {
      const heroScale = calculateHeroScale('splashHero', config, window.customAssets || {});
...
class EndScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (config.layout?.showEndHero && this.textures.exists('endHero')) {
      const heroScale = calculateHeroScale('endHero', config, window.customAssets || {});
...
window.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'UPDATE_ASSETS') {
    ...
    // IMPORTANT: Use window.customAssets (not __customAssets) to match what Preloader reads
