(function () {
    ...
    var pendingInit = null; // stores init function if MRAID needs to gate it

    /**
     * MRAID-aware CTA click handler.
     * Uses mraid.open() on MRAID networks, falls back to window.open() for GAM.
     */
    function ctaClick(url) {
        url = url || window.landingPageUrl;
        if (isMraid) {
            try {
                mraid.open(url);
            } catch (e) {
                console.warn('[MRAID] mraid.open() failed, falling back to window.open', e);
    ...
    /**
     * MRAID lifecycle: wait for SDK ready + viewable before starting the ad.
     */
    function getMraidState(initFn) {
    ...
    function onSdkReady(initFn) {
        mraid.addEventListener('viewableChange', function (viewable) {
            if (viewable && !mraidReady) {
                mraidReady = true;
                if (initFn) initFn();
            }
        });
    ...
    function checkMraid(initFn) {
        if (!isMraid) {
            console.warn('[MRAID] mraid not found, running in GAM/fallback mode');
            initFn();
            return;
        }
        console.log('[MRAID] mraid detected, initializing lifecycle');
        getMraidState(initFn);
    }

    /**
     * MRAID audio volume change listener.
     * Adjusts AudioContext gain when the platform reports volume changes.
     */
    function setupMraidAudioVolumeChange() {
        if (!isMraid) return;
        try {
            mraid.addEventListener('audioVolumeChange', function (volumePercentage) {
                if (volumePercentage !== null && audioCtx) {
                    // volumePercentage is 0-100, we can use it to control gain
                    var newVolume = volumePercentage / 100;
                    // Apply to all future sound playback via sndVolumes multiplier
                    window._mraidVolume = newVolume;
                }
            });
        } catch (e) {
            console.warn('[MRAID] audioVolumeChange listener failed', e);
        }
    }
    ...
    function loadImg(key, src) {
        var img = new Image();
        img.onload = function () { images[key] = img; loaded++; updateProgress(); if (loaded === total) checkMraid(init); };
        img.onerror = function () { console.warn('Missing: ' + src); images[key] = null; loaded++; updateProgress(); if (loaded === total) checkMraid(init); };
        img.src = src;
    }
    ...
    for (var i = 0; i < AUDIO_KEYS.length; i++) {
        (function (key) {
            ...
            fetch(url).then(function (r) {
                ...
                sndRawData[key] = arr;
                loaded++; updateProgress(); if (loaded === total) checkMraid(init);
            }).catch(function (e) {
    ...
    function playSound(key) {
        ...
        var vol = sndVolumes[key] || 1;
        // Apply MRAID volume multiplier if present
        if (typeof window._mraidVolume === 'number') {
            vol *= window._mraidVolume;
        }
        gain.gain.value = vol;
        src.connect(gain);
    ...
    function showEndOverlay() {
        ...
        ctaEl.onclick = function () { ctaClick(window.landingPageUrl); };
    ...
    function onDown(e) {
        ...
        if (state === 'end') {
            ...
            ctaClick(window.landingPageUrl);
            return;
        }
    ...
    function init() {
        ...
        // Setup MRAID audio volume change listener
        setupMraidAudioVolumeChange();
